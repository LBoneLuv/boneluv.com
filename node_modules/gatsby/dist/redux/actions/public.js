...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (process.env.NODE_ENV !== `test`) {
    if (pageComponentCache[page.component]) {
      ...
      if (isWindows) {
        ...
        page.component = segments.shift().toUpperCase() + `/` + segments.join(`/`);
      }
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!fileContent.includes(`export default`) && !fileContent.includes(`module.exports`) && !fileContent.includes(`exports.default`) && !fileContent.includes(`exports["default"]`) && // this check only applies to js and ts, not mdx
